...
from dsets1 import LunaDataset, getCt, getCandidateInfoDict, getCandidateInfoList, CandidateInfoTuple
...
logging.getLogger('dsets1').setLevel(logging.WARNING)
...
class NoduleAnalysisApp:
    ...
    def initModels(self):
        ...
        model_cls = getattr(model1, self.cli_args.cls_model)
        cls_model = model_cls()
        ...
        if self.cli_args.malignancy_path:
            model_cls = getattr(model1, self.cli_args.malignancy_model)
    ...
    def main(self):
        ...
        for _, series_uid in series_iter:
            ...
            if series_uid in candidateInfo_list:
