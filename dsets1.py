...
def getCandidateInfoList(requireOnDisk_bool=True):
    ...
    # the subsets yet.
    mhd_list = glob.glob(root_path + 'LUNADataset/subset*/*.mhd')
    presentOnDisk_set = {os.path.split(p)[-1][:-4] for p in mhd_list}

    candidateInfo_list = []
    with open(root_path + 'annotations_with_malignancy.csv', "r") as f:
    ...
    with open(root_path + 'candidates.csv', "r") as f:
        for row in list(csv.reader(f))[1:]:
            series_uid = row[0]
...
class Ct:
    def __init__(self, series_uid):
        mhd_path = glob.glob(
            root_path + 'LUNADataset/subset*/{}.mhd'.format(series_uid)
        )[0]
